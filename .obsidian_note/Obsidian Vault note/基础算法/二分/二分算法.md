...
3.推荐用开区间：
![配图](../../../截图文件/Pasted%20image%2020260423120923.png)
**好处**：
...
开区间 ： $l + 1 < r$;
闭区间 ：$l < r$;
这里就是常常再写闭区间中，容易发生的错误，再闭区间中的端点的边界位置处理时，没有正确的加1减1，或者忘记加1减1，导致再 $l + 1 == r$的状态时，发生死循环情况；
但是再闭区间没有可以很好规避这种问题，再 $l + 1 == r$ 处时，因为这已经跳出循环了，也就规避了发生死循环情况；

## 开区间写法：（推荐）
![配图](../../../截图文件/Pasted%20image%2020260423125228.png)
```c++
class Solution {
public:
